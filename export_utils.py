...
def transaction_file_name(location):
    ...
    :return: str
    '''
    return 'mlr.{0}.{1}.{2}'.format(location.get('agencyCode', ''), location.get('siteNumber', '').rstrip(), re.sub(DATE_TRANSFORM_REGEX, '', location.get('updated', '')))
...
def upload_to_s3(payload, destination_key, bucket, region, endpoint):
